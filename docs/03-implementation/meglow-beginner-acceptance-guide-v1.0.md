...
- 这里是**验证码登录**，不是用户名密码登录
- 必须先点击一次 `Send Code`，后端才会为这个手机号生成一条可用验证码记录
- 当前开发验证码有效期是 **60 秒**
- 当前开发验证码是**单次使用**，成功登录一次后，若要再次调用登录接口，需要重新获取一次验证码
...
如果提示 `invalid verification code`，优先检查这几项：

- 是否先点过 `Send Code`
- 是否在点击 `Send Code` 后 **60 秒内**完成登录
- 是否这条验证码已经被之前的一次成功登录消耗掉了
...
2. 等待上一步小程序登录所用验证码过期，或重新为同一手机号获取一次验证码
3. 找到 `POST /auth/send-code`，用同一手机号调用一次
4. 再找到 `POST /auth/login`，用同一手机号和验证码 `123456` 调用
5. 复制返回结果里的 `accessToken`

说明：

- 不要直接复用刚才小程序登录时已经用过的那次验证码
- 如果刚点过 `Send Code` 就在 Swagger 再点一次，可能会因为 60 秒限流而失败；这种情况下等一分钟再试即可
